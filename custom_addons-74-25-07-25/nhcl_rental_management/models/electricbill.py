...
class ElectricBill(models.Model):
    ...
    #         raise UserError("Please set the date on the Electric Bill.")
    #
    #     bill_month = self.date.strftime('%b').lower()[:3]  # Convert to 'may', 'jun', etc.
    #
    #     for line in self.line_ids:
    #         # Find matching CategoryBill by tenant and month and is
    #         category_bill = self.env['catagory.bill'].search([
    #             ('ten', '=', line.tenant_id.id),
    #             ('months', '=', bill_month), ('is_invoice_created', '=', False)
    #         ], limit=1)
    #
    #         if not category_bill:
    #             # Optionally, create one — or just skip with a warning
    #             continue
    #
    #         # Match category.line by date
    #         category_line = category_bill.ct_line_ids.filtered(lambda l: l.dt == self.date)
    #
    #         if category_line:
    #             category_line = category_line[0]
    #             category_line.open_unit = line.open_unit
    #             category_line.close_unit = line.close_unit
    #             category_line.reset_unit = line.reset_date
    #             # consumption auto-computes
...
class ElectricBillLine(models.Model):
    ...
    # def action_save_and_next(self):
    #     """Save current line and enable the next editable line."""
    #     self.ensure_one()
    #
    #     # STEP 1: Check if invoice is already created
    #     if self.tenant_id: #and self.date:
    #         month_key = self.bill_id.date.strftime('%b').lower()
    #         cat_bill = self.env['catagory.bill'].search([
    #             ('ten', '=', self.tenant_id.id),
    #             ('months', '=', month_key),
    #             ('is_invoice_created', '=', True)
    #         ], limit=1)
    #
    #         if cat_bill:
    #             raise UserError("Invoice already created for this tenant and month. You can't modify the bill.")
    #
    #     # STEP 2: Perform save and move to next editable line
    #     lines = self.bill_id.line_ids.sorted(key=lambda l: l.id)
    #     current_found = False
    #     moved = False
    #
    ...
    #                 if lines[j].editable_line:
    #                     lines[j].editable_line = True
    #                     moved = True
    #                     break
    #             break
...
class GasScreen(models.Model):
    ...
    #
    #         if not category_bill:
    #             print("No category bill found for tenant:", line.tenant_id.name)
    #             continue
    #
    #         category_line = category_bill.gas_line_ids.filtered(lambda l: l.date and l.date == self.date)
    #
    #         if category_line:
    #             category_line.write({
    ...
    #             })
    #             print(
    #                 f"Updated line: {category_line.opening_unit}, {category_line.closing_unit}, {category_line.reset_unit}")
    #         else:
    #             print("No matching gas line found for date:", self.date)
    def action_push_lines_to_gas_bill(self):
