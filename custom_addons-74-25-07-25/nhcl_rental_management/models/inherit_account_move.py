...
class AccountMove(models.Model):
    ...
    # def write(self, vals):
    #     if 'billing_from' in vals or 'nhcl_invoice_type' in vals:
    #         for record in self:
    #             billing_from = fields.Date.from_string(vals.get('billing_from', record.billing_from))
    #             nhcl_invoice_type = vals.get('nhcl_invoice_type', record.nhcl_invoice_type)
        ...
        if self.invoice_line_ids:
            ...
            for record in self:
                if record.billing_from and record.billing_to and record.tenancy_id and record.invoice_line_ids and record.nhcl_invoice_type == 'rent':
                    ...
                    if matched_cam:
                        # print('matchrecord data found')

                        cam_amount = matched_cam.cam_amount
                        # print(cam_amount, "cam_amount", cam_amount)
                        # print(month_days)
                        # print(cam_amount)
                        # print(billed_days)
    ...
    def _generate_electric_lines(self):
        for move in self:
            ...
            # previous_month = move.get_previous_month(current_month)
                ...
                for i, line in enumerate(hw_lines):
                    if line.opening_unit < 1:
                        if i > 0:
                            ...
                            break
                print(hw_lines, "....", first_line, "???", last_line, 'resetline===', reset_line)
            ...
            # this is for the water ghmc line created
    ...
    #     print("Starting action_post for invoice:", self.name)
    #     res = super(AccountMove, self).action_post()
    #     print("Super method executed successfully.")
    #
    ...
    #             ], limit=1)
    #
    #             print("Invoices fetched: Rent =", len(rent_invoices), ", RS =", bool(rs_invoice), ", SB =",
    #                   bool(sb_invoice), ", MG =", bool(mg_invoice))
    #
    ...
    #
    #                 all_rent_invoices = self.env['account.move'].search([
    #                     ('tenancy_id', '=', invoice.tenancy_id.id),
    #                     ('move_type', '=', 'out_invoice'),
    #                     ('state', '!=', 'cancel'),
    #                 ])
    #
    #                 for inv in all_rent_invoices:
    #                     print(f"Checking invoice: {inv.name} [{inv.nhcl_invoice_type}]")
    #
    #                     if invoice_type == 'rs':
    #                         if self.rs_amount == 0:
    ...
    #                     msg = f"RS amount ({rs_price}) is less than Rent amount ({rent_price})"
    #                     print("Validation failed:", msg)
    #                     raise UserError(_(msg))
    #
    #             elif invoice.nhcl_invoice_type == 'mg':
    #                 if mg_price < rent_price:
    #                     msg = f"MG amount ({mg_price}) is less than Rent amount ({rent_price})"
    #                     print("Validation failed:", msg)
    #                     raise UserError(_(msg))
    #
    #             elif invoice.nhcl_invoice_type == 'slb':
    #                 if sb_price < rent_price:
    #                     msg = f"SB amount ({sb_price}) is less than Rent amount ({rent_price})"
    #                     print("Validation failed:", msg)
    #                     raise UserError(_(msg))
    #
    ...
    #                     raise UserError(_(f"Rent amount ({rent_price}) is less than MG amount ({mg_price})"))
    #
    #             elif invoice.nhcl_invoice_type == 'rs' and rs_price < rent_price:
    ...
    def action_post(self):
        for invoice in self:
            if invoice.name in [False, '/']:
                print('condition satisfied: Generating invoice number...')
                ...
                if val.end_year == today:  # and not val.is_invoice_generated:
                    ...
                    invoice_vals_1 = {
                        ...
                        'invoice_line_ids': [(0, 0, {
                            ...
                            'quantity': 1,
                            'price_unit': 0,
                            'nhcl_area_units': rec.chargeable_area
                        })],
                    }
    ...
    def get_and_update_cam_invoice_data(self):
        ...
        for rec in ten_cam:
            ...
            for cam_line in rec.cam_year_ids:
                ...
                if rec.use_carpet:
                    nhcl_area_units = rec.carpet_area
    ...
    def _copy_recurring_entries(self):
        ...
        for record in self:
            ...
            for data in record.tenancy_id.year_ids:
                print(data.end_year.month, "///", next_date.month, "////", data.end_year.year, "???", next_date.year)
                print()
