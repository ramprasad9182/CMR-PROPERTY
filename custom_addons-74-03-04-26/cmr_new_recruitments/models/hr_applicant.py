...
class HrApplicant(models.Model):
    ...
    l10n_in_uan = fields.Char(string='UAN', size=12, copy=False)
    l10n_in_pan = fields.Char(string='PAN', size=10, copy=False)
    l10n_in_esic_number = fields.Char(string='ESIC Number', size=17, copy=False)
    ...
    def toggle_active(self):
        ...
        # --------------------------
        # AUTO UPPERCASE (UI LEVEL)
        # --------------------------

    @api.onchange('l10n_in_pan')
    def _onchange_pan_upper(self):
        if self.l10n_in_pan:
            self.l10n_in_pan = self.l10n_in_pan.upper()

    # --------------------------
    # VALIDATION ONLY
    # --------------------------
    @api.constrains('l10n_in_pan', 'l10n_in_uan', 'l10n_in_esic_number')
    def _check_indian_statutory_numbers(self):
        for rec in self:

            # PAN Validation
            if rec.l10n_in_pan:
                if not re.match(r'^[A-Z]{5}[0-9]{4}[A-Z]$', rec.l10n_in_pan):
                    raise ValidationError(_("Invalid PAN format. Example: ABCDE1234F"))

            # UAN Validation
            if rec.l10n_in_uan:
                if not re.match(r'^[0-9]{12}$', rec.l10n_in_uan):
                    raise ValidationError(_("UAN must be exactly 12 digits."))

            # ESIC Validation
            if rec.l10n_in_esic_number:
                if not re.match(r'^[0-9]{17}$', rec.l10n_in_esic_number):
                    raise ValidationError(_("ESIC Number must be exactly 17 digits."))
    ...
    def create_employee_from_applicant(self):
        for applicant in self:
            ...
            employee_vals = {
                ...
                'private_phone': applicant.partner_mobile,
                ...
                'l10n_in_uan': applicant.l10n_in_uan,
                'l10n_in_pan': applicant.l10n_in_pan,
                'l10n_in_esic_number': applicant.l10n_in_esic_number,
                'experience_type': applicant.experience_type,
                'education_copies': applicant.education_copies,
                # 'experience_docs': applicant.experience_docs,
                'experience_docs_ids': [(6, 0, applicant.experience_docs_ids.ids)],
            }
...
class ExperienceDocument(models.Model):
    _name = 'experience.document'
    _description = 'Experience Documents'
