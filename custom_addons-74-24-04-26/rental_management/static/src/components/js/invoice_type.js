...
export class OwlInvoiceType extends Component {
    ...
    async onStateChange(ev) {
        ...
        this.state.selectedStatus = value; // ✅ keep as string
        await this.loadRecordsForSelectedType();
    }
    ...
    async loadRecordsForSelectedType() {
        try {
            ...
            if (
                ...
                this.state.selectedTenant !== "All"
            ) {
                domain.push([
                    "partner_id",
                    "=",
                    parseInt(this.state.selectedTenant),
                ]);
            }
            // status filter
            if (
                this.state.selectedStatus &&
                this.state.selectedStatus !== "All States"
            ) {
                domain.push(["state", "=", this.state.selectedStatus]);
            }
            // Year filter (skip if “All”)
            if (this.state.selectedYear && this.state.selectedYear !== "All") {
                const year = parseInt(this.state.selectedYear, 10);
    ...
    async loadYears() {
        const records = await this.orm.searchRead(
            "account.move",
            [], // fetch all invoices or limit if needed
            ["invoice_date"],
        );
        ...
        records.forEach((rec) => {
            if (rec.invoice_date) {
                yearSet.add(new Date(rec.invoice_date).getFullYear());
            }
        });
        ...
        this.state.years = ["All", ...sortedYears];

        // Default selected year
        this.state.selectedYear = "All";
    }
    async loadStatus() {
        const status = await this.orm.searchRead("account.move", [], ["state"]);

        // Extract unique states
        const uniqueStates = [...new Set(status.map((rec) => rec.state))];

        // Add "All States" as the first option
        this.state.status = ["All States", ...uniqueStates];
    ...
    async getInvoiceCount(stages) {
        return this.orm.searchCount("account.move", [
            ["nhcl_invoice_type", "in", stages],
        ]);
    }
    // view action
    openRecord(id) {
