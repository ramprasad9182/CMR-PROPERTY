...
//it's for the setting the value of till date
...
rows.forEach(row => {
    ...
    if (fromDateInput) fromDateInput.setAttribute("required", "required");

    if (toDateInput && tillInput) {
        if (tillInput.checked) {
            toDateInput.removeAttribute("required");   // ✅ not mandatory
        } else {
            toDateInput.setAttribute("required", "required"); // ✅ mandatory
        }
    }
});
    ...
    window.addProfessionalEntry = function () {
        ...
        wrapper.innerHTML = `
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="years_experience_${index}" class="form-control years_experience_0" step="0.1" min="0"/>
            </div>
            <div class="col-md-4 mt-2">
                <label>From Date:</label>
                <input type="date" name="from_date_${index}" class="form-control from_date_0"/>
            </div>
            <div class="col-md-4 mt-2">
                <label>To Date:</label>
                <input type="date" name="to_date_${index}" class="form-control to_date_0"/>
            </div>
...
//        setupRealTimeDateValidation();
    ...
    const rows = document.querySelectorAll(".professional-entry");
    console.clear(); // just to keep logs clean
    ...
    rows.forEach((row, index) => {
        ...
        console.log("From:", fromInput?.value, "To:", toInput?.value, "Till:", tillInput?.checked);

        if (fromInput && (toInput || tillInput)) {
            let fromDate = new Date(fromInput.value);
            let toDate   = tillInput.checked ? new Date() : new Date(toInput.value);

            if (fromDate instanceof Date && !isNaN(fromDate) &&
                toDate instanceof Date && !isNaN(toDate)) {

                let diffTime = toDate - fromDate;
                let diffYears = diffTime / (1000 * 60 * 60 * 24 * 365); // rough years

                yearsInput.value = diffYears.toFixed(1);
                console.log(`✅ Experience for Row ${index + 1}: ${diffYears.toFixed(1)} years`);
            } else {
                yearsInput.value = "";
                console.log("⚠️ Invalid date(s)");
            }
        }
    });
}
