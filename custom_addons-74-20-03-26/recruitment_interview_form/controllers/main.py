...
class InterviewFormController(http.Controller):
    ...
    def submit_interview_form(self, **post):
        ...
        try:
            ...
            form_id = int(post.get('form_id') or 0)
            name = post.get('name')
            ...
            # SIMPLE FIX: force contact name = candidate name
            if candidate.partner_id:
                candidate.partner_id.sudo().write({'name': name})
            ...
            # --------------------------------------------------
            # BLOCK IF REFUSED WITHIN LAST 3 MONTHS
            # --------------------------------------------------
            refused_applicant = request.env['hr.applicant'].sudo().search([
                '|',
                ('email_from', '=', email),
                ('partner_mobile', '=', partner_mobile),
                ('active', '=', False),
                ('refuse_date', '!=', False),
            ], order='refuse_date desc', limit=1)

            if refused_applicant:
                # Convert refuse_date (datetime) → date
                refuse_date = refused_applicant.refuse_date
                if isinstance(refuse_date, datetime):
                    refuse_date = refuse_date.date()

                block_till = refuse_date + relativedelta(months=3)

                if today < block_till:
                    return request.render('recruitment_interview_form.error_page', {
                        'error_msg': "Your application already exist in our database."
                    })
            # --------------------------------------------------
            # 2️⃣ CHECK DUPLICATE BY EMAIL + JOB + DEPARTMENT
            # --------------------------------------------------
            email_exists = request.env['hr.applicant'].sudo().search([
                ('email_from', '=', email),
                ('job_id', '=', job_id),
            ...
            # --------------------------------------------------
            # 3️⃣ CHECK DUPLICATE BY MOBILE + JOB + DEPARTMENT
            # --------------------------------------------------
            mobile_exists = request.env['hr.applicant'].sudo().search([
                ('partner_mobile', '=', partner_mobile),
                ('job_id', '=', job_id),
                ('department_id', '=', department_id),
            ], limit=1)
            ...
            company = candidate.company_id or request.env.company
